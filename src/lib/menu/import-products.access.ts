...
function membershipHasMenuRead(membership: MembershipWithOrg) {
  return (
    membershipHasPermission(membership, PERMISSIONS.MENU_READ) ||
    membershipHasPermission(membership, PERMISSIONS.MENU_WRITE)
  );
}

function membershipHasMenuWrite(membership: MembershipWithOrg) {
  return membershipHasPermission(membership, PERMISSIONS.MENU_WRITE);
}

export async function requireMenuImportWriteAccess(restaurantId: string) {
  ...
  if (!targetMembership || !membershipHasMenuWrite(targetMembership)) {
...
export async function resolveAccessibleSourceRestaurantIds(
  ...
  for (const membership of memberships) {
    if (!membershipHasMenuRead(membership)) {
      continue;
    }
...
export async function getTargetRestaurantOrganizationId(restaurantId: string) {
